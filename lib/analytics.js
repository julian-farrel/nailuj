 ...
 */
export function computePortfolioMetrics(assetMetricsMap, weights, riskFreeRate = RISK_FREE_RATE) {
...
/**
 * Compute a full Pearson correlation matrix for a set of assets.
 *
 * @param {string[]} tickers - Ordered list of asset tickers
 * @param {Object} dailyReturnsMap - { ticker: number[] } — arrays of daily returns
 * @returns {Object} Nested map: { tickerA: { tickerB: r, ... }, ... }
 *
 * For each pair (A, B):
 *   r = Cov(A,B) / (σ_A * σ_B)
 * Values are aligned to the shortest common window.
 */
export function computePearsonMatrix(tickers, dailyReturnsMap) {
  const result = {};
  ...
  for (const a of tickers) {
    ...
    for (const b of tickers) {
      ...
      const ra = dailyReturnsMap[a];
      const rb = dailyReturnsMap[b];
      ...
      // Align to shortest window (take tail of longer series)
      const len = Math.min(ra.length, rb.length);
      const A = ra.slice(ra.length - len);
      const B = rb.slice(rb.length - len);
      ...
      result[a][b] = denom === 0 ? null : +(num / denom).toFixed(6);
    }
  }
